...
module.exports = {
    ...
    doubleNumberQuery: async function(start, end, web3) {
        ...
        while (currentBlock >= start){
            ...
            // make a query to the blockchain to fetch the current most recent block (caveat: potential to case race condition --> filter function would be better hSere)
            ...
            totalTransactionCount += transactionData.transactionCount;
        ...
        if (totalTransactionCount !== 0) {
            ...
            console.log("Total percent of transactions that are made by contract addresses: " + ((totalContracts/(totalTransactionCount))*100).toFixed(2) + "%"); // assuming that making a transaction means sending or receiving
        }
    },
}
